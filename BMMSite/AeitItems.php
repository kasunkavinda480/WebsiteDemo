...
<?php
	...
	$result_count = mysqli_query($link,"SELECT COUNT(*) As total_records FROM product WHERE AdminName Like '%".$AdminName."%'");
	...
	$result = mysqli_query($link,"SELECT * FROM product WHERE AdminName like '%".$AdminName."%'  LIMIT $offset, $total_records_per_page");
	$no = '0';
...
?>
	...
	<td id="itmeNameCol" ><?php  echo '<a href="AEditSingalProduct.php?Pid=',$Orderid,'">'?><?php  echo "".$row['PName']."" ?>   </a></td>
			<td>
		<?php echo "".$row['Price']."" ?>
			<br>
			<?php echo "".$row['PDiscount']."" ?>
				<br>
			<?php echo "".$row['Stock']."" ?>
		</td>
